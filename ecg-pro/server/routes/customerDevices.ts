...
import { secretMatches } from "../services/deviceAuthService";
...
}).strict().refine((value) => value.name !== undefined || value.meterConstant !== undefined);
...
export async function customerClaimDevice(req: IncomingMessage, res: ServerResponse) {
  ...
  if (!parsed.success) throw new HttpError(400, "Expected deviceId and optional claimCode");
  ...
  if (device.ownership) {
    if (device.ownership.userId !== auth.user.id) throw new HttpError(409, "Device is already claimed");
    json(res, 200, serializeDevice(device));
    return;
  }

  if (device.claimCodeHash) {
    if (!parsed.data.claimCode || !secretMatches(parsed.data.claimCode, device.claimCodeHash)) {
      throw new HttpError(403, "Invalid device claim code");
    }
  } else if (process.env.ALLOW_INSECURE_DEVICE_CLAIM !== "true") {
    throw new HttpError(403, "This device does not have a claim code configured");
  }
...
export async function customerListDevices(req: IncomingMessage, res: ServerResponse) {
  ...
  json(res, 200, { devices: rows.map((row) => serializeDevice(row.device)) });
}

export async function customerDeviceState(req: IncomingMessage, res: ServerResponse, hardwareId: string) {
  if (req.method !== "GET") throw new HttpError(405, "Method not allowed");
  const auth = await authorizeCustomer(req);
...
export async function customerUpdateDevice(req: IncomingMessage, res: ServerResponse, hardwareId: string) {
  if (req.method !== "PATCH") throw new HttpError(405, "Method not allowed");
  const auth = await authorizeCustomer(req);
  const ownership = await ownedDevice(auth.user.id, hardwareId);
  const parsed = updateSchema.safeParse(await readJson(req));
  if (!parsed.success) throw new HttpError(400, "Expected name and/or meterConstant");
...
export async function customerRemoveDevice(req: IncomingMessage, res: ServerResponse, hardwareId: string) {
