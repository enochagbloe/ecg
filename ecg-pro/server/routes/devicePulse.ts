...
export async function devicePulse(req: IncomingMessage, res: ServerResponse) {
  ...
  if (!parsed.success) throw new HttpError(400, "Expected only deviceId, pulse (0 or 1), a safe non-negative integer sequence, and ISO timestamp");
  const result = await processPulse(store, parsed.data, token);
  // The transaction promise resolves only after commit.
  const event = pulseBroadcast(result);
  if (event) getIO().emit("pulseData", event);
